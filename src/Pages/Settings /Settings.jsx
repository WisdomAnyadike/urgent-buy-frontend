...
const Settings = () => {
  ...
  useEffect(() => {
    const fetchUserTransactions = async () => {
      try {
        const res = await axios.get('https://ecommerce-backend-2-ykz2.onrender.com/Api/Transaction/getUserTransactions', {
          headers: {
  ...
  const changePassword = async (event) => {
    ...
    if (!passwordRegex.test(password) || !passwordRegex.test(confirmPassword)) {
      ...
      try {
        const res = await axios.post('https://ecommerce-backend-2-ykz2.onrender.com/Api/User/editPassword', { Password: password }, {
          headers: {
            'Authorization': `Bearer ${token}`,
            "content-type": "application/json"
          }
  ...
  const changeUserInfo = async (event) => {
    ...
    try {
      const res = await axios.post('https://ecommerce-backend-2-ykz2.onrender.com/Api/User/editUserInfo', userObj, {
        headers: {
          'Authorization': `Bearer ${token}`,
          "content-type": "application/json"
        }
  ...
  const handleDelete = async (e) => {
    ...
    if (verifyDelete) {
      try {
        const res = await axios.post('https://ecommerce-backend-2-ykz2.onrender.com/Api/User/deleteUser', {}, {
          headers: {
            'Authorization': `Bearer ${token}`,
            "content-type": "application/json"
          }
